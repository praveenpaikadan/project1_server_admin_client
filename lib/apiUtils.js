...
var ipAddress = require('ip').address();
const PORT = process.env.PORT || 3000
const APP_API_BASE_URL = (process.env.BASE_URL || `http://${ipAddress}:${PORT}`) + `/api/v1` 
const ADMIN_API_BASE_URL = (process.env.BASE_URL || `http://${ipAddress}:${PORT}`) +  `/api/admin`
const API_BASE_URL = (process.env.BASE_URL || `http://${ipAddress}:${PORT}`) + `/api`
